...
export class ListarSyllabusComponent implements OnInit {
  ...
  changeDataTableSyllabus() {
    ...
    this.request.get(environment.SYLLABUS_CRUD, `syllabus?query=espacio_academico_id:${this.EspacioAcademico.asi_cod},syllabus_actual:true,activo:true`).subscribe({
      next:(dataSyllabus) => {
        if (dataSyllabus) {
          ...
          if(this.syllabus.length==0){
            ...
            if (this.hasSyllabusByProjectPlan) {
              ...
              Swal.fire({
                icon: 'warning',
                title: 'El Proyecto / Programa Curricular o Plan de estudios no tiene syllabus asignado',
  ...
  filterSyllabusByProjectPlan() {
    ...
    this.syllabus.forEach((value) => {
      ...
      if (proyecto_ids != undefined && plan_ids != undefined && proyecto_ids.includes(pen_cra_cod) && plan_ids.includes(pen_nro)){
  ...
  validateMultipleOwnerSyllabus(){
    ...
    if(this.Syllabus.proyecto_curricular_ids == undefined || this.Syllabus.proyecto_curricular_ids.length == 0){
      return false;
    } else if(this.Syllabus.proyecto_curricular_ids.includes(pen_cra_cod) && this.Syllabus.proyecto_curricular_ids.length == 1){
      return false;
    }

    if(this.Syllabus.plan_estudios_ids == undefined || this.Syllabus.plan_estudios_ids.length == 0){
      return false;
    } else if(this.Syllabus.plan_estudios_ids.includes(pen_nro) && this.Syllabus.plan_estudios_ids.length == 1){
      return false;
    }
    return true;
  }
  ...
  updateVersionSyllabus(syllabus: any){
    ...
    this.request.put(environment.SYLLABUS_CRUD, 'syllabus', syllabus, syllabus._id).subscribe({
      next: (respuesta: any) => {
  ...
  shouldAddSyllabusButton(row: number): boolean {
    ...
    if(syllabusRow.proyecto_curricular_ids == undefined || syllabusRow.proyecto_curricular_ids.length == 0 || syllabusRow.plan_estudios_ids == undefined || syllabusRow.plan_estudios_ids.length == 0){
      return true;
    } else if(syllabusRow.proyecto_curricular_ids.includes(pen_cra_cod) && syllabusRow.plan_estudios_ids.includes(pen_nro)){
      return false;
    } 
    return true;
  }
  ...
  addProjectPlan2Syllabus(row: number){
    Swal.fire({
      ...
      if(result.isConfirmed){
        ...
        if(syllabus.proyecto_curricular_ids == undefined || syllabus.proyecto_curricular_ids.length == 0){
        ...
        if(syllabus.plan_estudios_ids == undefined || syllabus.plan_estudios_ids.length == 0){
    ...
    Swal.fire({
      ...
      if(result.isConfirmed){
        ...
        if(syllabus.plan_estudios_ids == undefined || syllabus.plan_estudios_ids.length == 0){
          ...
          syllabus.plan_estudios_ids = syllabus.plan_estudios_ids.filter(pe_id => pe_id !== pen_nro);
        }
        this.updateVersionSyllabus(syllabus);
      }
    })
  ...
  shouldRemoveSyllabusButton(row: number): boolean {
    ...
    if(syllabusRow.proyecto_curricular_ids == undefined || syllabusRow.proyecto_curricular_ids.length == 0){
      return false;
    } else if(syllabusRow.proyecto_curricular_ids.includes(pen_cra_cod) && syllabusRow.plan_estudios_ids.includes(pen_nro)){
      return true;
    } 
